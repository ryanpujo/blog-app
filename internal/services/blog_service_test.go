...
func (m *MockBlogRepository) Create(blog models.BlogPayload) (*uint, error) {
	args := m.Called(blog)
...
func (m *MockBlogRepository) FindBlogs() ([]*models.Blog, error) {
	args := m.Called()
	return args.Get(0).([]*models.Blog), args.Error(1)
}
...
func (m *MockBlogRepository) Update(id uint, payload models.BlogPayload) error {
...
func Test_blogService_Create(t *testing.T) {
	...
	for name, tc := range testingTable {
		t.Run(name, func(t *testing.T) {
			...
			id, err := blogService.Create(models.BlogPayload{})
...
func Test_blogService_FindById(t *testing.T) {
	testTable := map[string]struct {
		...
		"success": {
			arrange: func() {
				mockBlogRepo.On("FindById", mock.Anything).Return(&models.Blog{Title: "test"}, nil).Once()
			},
			assert: func(t *testing.T, actualBlog *models.Blog, err error) {
				require.NoError(t, err)
		...
		"failed": {
			arrange: func() {
				mockBlogRepo.On("FindById", mock.Anything).Return((*models.Blog)(nil), errors.New("failed")).Once()
			},
			assert: func(t *testing.T, actualBlog *models.Blog, err error) {
				require.Error(t, err)
				require.Equal(t, "failed", err.Error())
...
func Test_blogService_FindBlogs(t *testing.T) {
	testTable := map[string]struct {
		...
		"success": {
			arrange: func() {
				mockBlogRepo.On("FindBlogs").Return([]*models.Blog{{Title: "test"}, {}}, nil).Once()
			},
			assert: func(t *testing.T, actualBlog []*models.Blog, err error) {
				require.NoError(t, err)
		...
		"failed": {
			arrange: func() {
				mockBlogRepo.On("FindBlogs").Return(([]*models.Blog)(nil), errors.New("failed")).Once()
			},
			assert: func(t *testing.T, actualBlog []*models.Blog, err error) {
				require.Error(t, err)
				require.Equal(t, "failed", err.Error())
...
func Test_blogService_Update(t *testing.T) {
	...
	for name, tc := range testTable {
		t.Run(name, func(t *testing.T) {
			...
			err := blogService.Update(1, models.BlogPayload{})
